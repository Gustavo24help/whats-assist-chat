...
BEGIN
  ...
  per_customer AS (
    SELECT canonical_id, COUNT(*) AS svc_count, SUM(valor) AS ltv
    FROM services GROUP BY canonical_id
  )
  ...
  no_periodo AS (
    SELECT DISTINCT canonical_id FROM services
    WHERE COALESCE(completed_at, service_date, created_at) BETWEEN p_start AND p_end
  ),
  com_historico AS (
    SELECT DISTINCT np.canonical_id FROM no_periodo np
    JOIN services s ON s.canonical_id = np.canonical_id
    WHERE COALESCE(s.completed_at, s.service_date, s.created_at) < p_start
  )
  ...
  SELECT COALESCE(SUM(s.valor), 0),
         COALESCE(SUM(s.valor) FILTER (WHERE t.svc_count >= 2), 0)
  INTO v_receita_total, v_receita_recorrente
  FROM services s JOIN totais t USING (canonical_id)
  WHERE COALESCE(s.completed_at, s.service_date, s.created_at) BETWEEN p_start AND p_end;

  WITH services AS (
    SELECT canonical_id, COALESCE(completed_at, service_date, created_at) AS dt
    FROM public.v_customer_services_enriched
    WHERE is_valid AND canonical_id IS NOT NULL
      AND (p_segment = 'all' OR segmento = p_segment)
  ),
  ranked AS (
    SELECT canonical_id, dt, ROW_NUMBER() OVER (PARTITION BY canonical_id ORDER BY dt) AS rn FROM services
  ),
  segundos AS (
  ...
  SELECT COALESCE(AVG(dias), 0),
         COALESCE(percentile_cont(0.5) WITHIN GROUP (ORDER BY dias)::numeric, 0)
  INTO v_dias_avg, v_dias_med FROM segundos;

  WITH per_customer AS (
    SELECT canonical_id, COUNT(*) AS svc_count,
           MAX(COALESCE(completed_at, service_date, created_at)) AS last_dt
    FROM public.v_customer_services_enriched
    WHERE is_valid AND canonical_id IS NOT NULL
      AND (p_segment = 'all' OR segmento = p_segment)
  ...
  WITH meses AS (
    SELECT generate_series(date_trunc('month', p_end) - interval '12 months',
                           date_trunc('month', p_end), interval '1 month') AS mes
  ),
  services AS (
    SELECT *, COALESCE(completed_at, service_date, created_at) AS dt
    FROM public.v_customer_services_enriched
    WHERE is_valid AND canonical_id IS NOT NULL
      AND (p_segment = 'all' OR segmento = p_segment)
  ),
  ...
  RETURN jsonb_build_object(
    ...
    'clientes_recorrentes', v_clientes_recorrentes,
    'recorrencia_geral_pct', CASE WHEN v_clientes_total > 0
      THEN ROUND(v_clientes_recorrentes::numeric * 100 / v_clientes_total, 2) ELSE 0 END,
    'clientes_periodo', v_clientes_periodo,
    'recorrentes_periodo', v_recorrentes_periodo,
    'recorrencia_periodo_pct', CASE WHEN v_clientes_periodo > 0
      THEN ROUND(v_recorrentes_periodo::numeric * 100 / v_clientes_periodo, 2) ELSE 0 END,
    'receita_total', v_receita_total,
    'receita_recorrente', v_receita_recorrente,
    'pct_receita_recorrente', CASE WHEN v_receita_total > 0
      THEN ROUND(v_receita_recorrente * 100 / v_receita_total, 2) ELSE 0 END,
    'cac_economizado', GREATEST(v_recorrentes_periodo, 0) * v_cac,
    'ltv_avg', ROUND(COALESCE(v_ltv_avg, 0), 2),
    'ltv_liq', ROUND(v_ltv_liq, 2),
    'ltv_cac', ROUND(COALESCE(v_ltv_cac, 0), 2),
    'dias_avg', ROUND(v_dias_avg, 1),
    'dias_med', ROUND(v_dias_med, 1),
...
RETURNS TABLE(
  ...
  WITH services AS (
    SELECT canonical_id, COALESCE(completed_at, service_date, created_at) AS dt, valor
    FROM public.v_customer_services_enriched
    WHERE is_valid AND canonical_id IS NOT NULL
      AND (p_segment = 'all' OR segmento = p_segment)
  ),
  ranked AS (
    SELECT canonical_id, dt, valor,
           ROW_NUMBER() OVER (PARTITION BY canonical_id ORDER BY dt) AS rn FROM services
  ),
  primeiros AS (
    SELECT canonical_id, dt AS first_dt, date_trunc('quarter', dt) AS cohort_q FROM ranked WHERE rn = 1
  ),
  segundos AS (SELECT canonical_id, dt AS second_dt FROM ranked WHERE rn = 2),
  ...
  SELECT
    ...
    cohort_q, COUNT(*)::int,
    CASE WHEN now() >= cohort_q + interval '30 days'
      THEN ROUND(100.0 * COUNT(*) FILTER (WHERE second_dt - first_dt <= interval '30 days') / NULLIF(COUNT(*),0), 2) END,
    CASE WHEN now() >= cohort_q + interval '60 days'
      THEN ROUND(100.0 * COUNT(*) FILTER (WHERE second_dt - first_dt <= interval '60 days') / NULLIF(COUNT(*),0), 2) END,
    CASE WHEN now() >= cohort_q + interval '90 days'
      THEN ROUND(100.0 * COUNT(*) FILTER (WHERE second_dt - first_dt <= interval '90 days') / NULLIF(COUNT(*),0), 2) END,
    CASE WHEN now() >= cohort_q + interval '180 days'
      THEN ROUND(100.0 * COUNT(*) FILTER (WHERE second_dt - first_dt <= interval '180 days') / NULLIF(COUNT(*),0), 2) END,
    CASE WHEN now() >= cohort_q + interval '365 days'
      THEN ROUND(100.0 * COUNT(*) FILTER (WHERE second_dt - first_dt <= interval '365 days') / NULLIF(COUNT(*),0), 2) END,
    ...
    ROUND((percentile_cont(0.5) WITHIN GROUP (
      ORDER BY (EXTRACT(EPOCH FROM (second_dt - first_dt))/86400.0)
    ) FILTER (WHERE second_dt IS NOT NULL))::numeric, 1),
...
RETURNS TABLE(
  ...
  primeiros AS (
    SELECT canonical_id, provider_id, provider_name, valor AS first_valor, nps_nota, total FROM ranked WHERE rn = 1
  ),
  ltv AS (SELECT canonical_id, SUM(valor) AS total_ltv FROM services GROUP BY canonical_id)
  SELECT
    COALESCE(p.provider_id, 'sem_prestador'),
    COALESCE(MAX(p.provider_name), '(sem prestador)'),
    COUNT(*)::int,
    COUNT(*) FILTER (WHERE p.total >= 2)::int,
    ROUND(100.0 * COUNT(*) FILTER (WHERE p.total >= 2) / NULLIF(COUNT(*),0), 2),
    ROUND(AVG(l.total_ltv), 2),
    ROUND(SUM(l.total_ltv) FILTER (WHERE p.total >= 2), 2),
    ROUND(AVG(p.nps_nota), 2),
    ROUND(AVG(p.first_valor), 2)
...
RETURNS TABLE(
  provider_id text, provider_name text,
  clientes_que_sumiram int, nps_avg_ultimo numeric, ticket_avg_ultimo numeric
) LANGUAGE sql STABLE SECURITY INVOKER SET search_path = public AS $$
  WITH services AS (
    SELECT *, COALESCE(completed_at, service_date, created_at) AS dt
    FROM public.v_customer_services_enriched
  ...
  dormentes AS (
    SELECT canonical_id, last_dt FROM per_customer
    WHERE total >= 2 AND last_dt < (now() - interval '180 days')
  ),
  ultimo AS (
  ...
  SELECT COALESCE(provider_id, 'sem_prestador'),
         COALESCE(MAX(provider_name), '(sem prestador)'),
...
CREATE OR REPLACE FUNCTION public.recurrence_reactivation_tags(
  ...
  per_customer AS (
    SELECT canonical_id, COUNT(*) AS total, SUM(valor) AS ltv, MAX(dt) AS last_dt FROM services GROUP BY canonical_id
  ),
  ranked AS (
...
GRANT EXECUTE ON FUNCTION public.recurrence_summary(timestamptz, timestamptz, text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.recurrence_cohorts(text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.recurrence_provider_first(text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.recurrence_provider_last_dormant(text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.recurrence_reactivation_tags(text, int, int) TO authenticated, service_role;
