...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND NEW.status IN ('Agendado', 'Finalizado') AND (OLD.status IS NULL OR OLD.status IS DISTINCT FROM NEW.status)) THEN
    
    -- Só processar se status é elegível (para INSERT também)
    IF NEW.status NOT IN ('Agendado', 'Finalizado') THEN
      RETURN NEW;
    END IF;
  ...
  IF TG_OP = 'UPDATE'
     AND NEW.status IN ('Agendado', 'Finalizado')
