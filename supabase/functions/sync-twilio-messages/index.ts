...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`🔍 Buscando mensagens da Twilio desde: ${dateSentAfter}`);
    ...
    for (const msgTwilio of mensagensTwilio) {
      try {
        ...
        console.log(`📥 Processando nova mensagem: ${msgTwilio.sid}`);
        ...
        if (error) {
          ...
          console.log(`✅ Mensagem salva: ${msgTwilio.sid} - ${texto.substring(0, 30)}`);
          mensagensNovas++;
        }
        ...
        erros++;
        errosDetalhados.push(`Erro geral: ${err instanceof Error ? err.message : 'Desconhecido'}`);
      }
    }
