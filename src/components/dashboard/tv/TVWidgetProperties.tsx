...
import { X, Lock, Unlock, AlignCenterHorizontal, AlignCenterVertical, RotateCcw, ArrowUp, ArrowDown, Trash2, Grid3X3, Save, Upload, Download, Eye, EyeOff, MoveUp, MoveDown, Repeat, ChevronDown, ChevronRight, EyeOff as EyeOffAll } from 'lucide-react';
...
export function TVWidgetProperties() {
  ...
  const someEnabled = widgets.some(w => w.enabled);

  const hideAllWidgets = () => {
    widgets.forEach(w => {
      if (w.enabled) updateWidget(w.id, { enabled: false });
    });
  };

  const showAllWidgets = () => {
    widgets.forEach(w => {
      if (!w.enabled) updateWidget(w.id, { enabled: true });
    });
  };
  ...
  return (
    <div className="fixed right-0 top-0 bottom-0 w-[280px] bg-gray-900/95 backdrop-blur-md border-l border-cyan-500/20 text-white z-50 overflow-y-auto flex flex-col">
        ...
        <div className="p-3 space-y-3 border-b border-gray-700/50">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <Input type="number" value={Math.round(selected.x)} onChange={e => updateWidget(selected.id, { x: Number(e.target.value) || 0 })} className="h-7 text-xs bg-gray-800 border-gray-600 text-white" />
            </div>
            <div>
              <label className="text-[10px] text-gray-500 block mb-0.5">Y (px)</label>
              <Input type="number" value={Math.round(selected.y)} onChange={e => updateWidget(selected.id, { y: Number(e.target.value) || 0 })} className="h-7 text-xs bg-gray-800 border-gray-600 text-white" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <Input type="number" value={Math.round(selected.width)} onChange={e => updateWidget(selected.id, { width: Math.max(100, Number(e.target.value) || 100) })} className="h-7 text-xs bg-gray-800 border-gray-600 text-white" />
            </div>
            <div>
              <label className="text-[10px] text-gray-500 block mb-0.5">Altura (px)</label>
              <Input type="number" value={Math.round(selected.height)} onChange={e => updateWidget(selected.id, { height: Math.max(60, Number(e.target.value) || 60) })} className="h-7 text-xs bg-gray-800 border-gray-600 text-white" disabled={selected.autoHeight} />
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Switch checked={selected.locked} onCheckedChange={v => updateWidget(selected.id, { locked: v })} className="h-4 w-7" />
            </div>
            <div className="flex items-center justify-between">
              <span className="text-[10px] text-gray-400">Altura automática</span>
              <Switch checked={selected.autoHeight} onCheckedChange={v => updateWidget(selected.id, { autoHeight: v })} className="h-4 w-7" />
            </div>
            <div className="flex items-center justify-between">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            <Button variant="outline" size="sm" className="h-7 text-[10px] bg-gray-800 border-gray-600 text-gray-300" onClick={() => centerHorizontal(selected.id)}>
              <AlignCenterHorizontal className="h-3 w-3 mr-1" /> Centralizar H
            </Button>
            <Button variant="outline" size="sm" className="h-7 text-[10px] bg-gray-800 border-gray-600 text-gray-300" onClick={() => centerVertical(selected.id)}>
              <AlignCenterVertical className="h-3 w-3 mr-1" /> Centralizar V
            </Button>
            <Button variant="outline" size="sm" className="h-7 text-[10px] bg-gray-800 border-gray-600 text-gray-300" onClick={() => bringToFront(selected.id)}>
              <ArrowUp className="h-3 w-3 mr-1" /> Para frente
            </Button>
            <Button variant="outline" size="sm" className="h-7 text-[10px] bg-gray-800 border-gray-600 text-gray-300" onClick={() => sendToBack(selected.id)}>
              <ArrowDown className="h-3 w-3 mr-1" /> Para trás
            </Button>
      ...
      {/* Widgets list / layers - COLLAPSIBLE */}
        ...
        <div className="p-3 border-b border-gray-700/50">
          <CollapsibleTrigger className="flex items-center justify-between w-full mb-2">
            <div className="text-[10px] text-gray-400 uppercase tracking-wider flex items-center gap-1">
              {widgetsOpen ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
              Camadas / Widgets ({widgets.filter(w => w.enabled).length}/{widgets.length})
            </div>
          </CollapsibleTrigger>

          {/* Hide/Show All button - always visible */}
          <div className="flex gap-1 mb-2">
            <Button
              variant="outline"
              size="sm"
              className="h-6 text-[9px] flex-1 bg-gray-800 border-gray-600 text-gray-300"
              onClick={someEnabled ? hideAllWidgets : showAllWidgets}
            >
              {someEnabled ? (
                <><EyeOff className="h-3 w-3 mr-1" /> Esconder Todos</>
              ) : (
                <><Eye className="h-3 w-3 mr-1" /> Mostrar Todos</>
              )}
            </Button>
          </div>
      ...
      <Collapsible open={saveLoadOpen} onOpenChange={setSaveLoadOpen}>
        <div className="p-3 border-b border-gray-700/50 space-y-2">
          <CollapsibleTrigger className="flex items-center w-full">
            <div className="text-[10px] text-gray-400 uppercase tracking-wider flex items-center gap-1">
              {saveLoadOpen ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
              Salvar / Carregar
            </div>
          </CollapsibleTrigger>
          <CollapsibleContent>
                ...
                <div className="space-y-1">
                  {savedLayouts.map((l, idx) => (
                    <div key={l.name} className="flex items-center gap-1">
                      ...
                      <button onClick={() => moveSavedLayout(l.name, 'up')} disabled={idx === 0} className="text-gray-500 hover:text-cyan-300 p-0.5 disabled:opacity-30" title="Subir">
                        <MoveUp className="h-3 w-3" />
                      </button>
                      <button onClick={() => moveSavedLayout(l.name, 'down')} disabled={idx === savedLayouts.length - 1} className="text-gray-500 hover:text-cyan-300 p-0.5 disabled:opacity-30" title="Descer">
      ...
      {/* Rotação de layouts - COLLAPSIBLE */}
      <Collapsible open={rotationOpen} onOpenChange={setRotationOpen}>
        <div className="p-3 border-b border-gray-700/50 space-y-2">
          <CollapsibleTrigger className="flex items-center w-full">
            <div className="text-[10px] text-gray-400 uppercase tracking-wider flex items-center gap-1">
              {rotationOpen ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
      ...
      <div className="p-3">
        <Button variant="outline" size="sm" className="h-7 text-xs w-full bg-gray-800 border-red-500/30 text-red-400 hover:bg-red-500/10" onClick={resetLayout}>
          <RotateCcw className="h-3 w-3 mr-1" /> Resetar Layout
        </Button>
      </div>
    </div>
  );
